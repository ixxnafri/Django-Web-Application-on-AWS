...
def get_posts(request):
    ...
    # memcached logic:
    # Multiple user can use the same laptop to do login. Thus, using the netid as the key and last search result as the value.
    # Put the key-value pair into the memcached and every pair will stay in the cache for 10 secs
    cache_time = 10  # time in seconds for cache to be valid
    #    netid = request.query_params.get('netid')
    cache_key = user.username  # needs to be unique
...
def follow(request):
    ...
    f = User.objects.filter(username=friend_name)[0]
